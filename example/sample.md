...
技术文档常带代码块。代码块里的 `式（1）`、`![[图.png]]`、`$x$`、分隔线、`#` 标题
都只是字面文字，预处理不得触碰；verify 的逐字比对也不应把代码里的中文算进正文。

```python
# 计算任务平均响应时间，推导见式（1）
config = {"figure": "![[demo.png]]", "cost": "$x^2$"}
```

```text
图 3.1 展示了整体流程（这是代码内容，不是正文，更不是题注）
---
# 5 这是注释不是章节标题
```

代码区之后的公式编号应继续顺延，不受上面那行伪标题影响：
...
<!-- 陷阱 9：代码区段。里面的引用、图片、公式定界符、分隔线、伪标题都是字面文字，
     一旦被当正文处理就是静默篡改；verify 若把代码中文算进正文比对则是误报 -->

## 2.5 题注与正文的边界

图 3.1 展示了本方法与基线的对比趋势——这是以「图 x.y」开头的正文段落，不是题注，
不应该被居中或改成小五号字体。

**图 2：不同权重下的收敛曲线**

<!-- 陷阱 10：「图 x.y」开头的正文段落不是题注。题注只认「图 N：」冒号形态 -->
